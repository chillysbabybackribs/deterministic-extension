...
const PLANNER_SYSTEM_PROMPT = [
  ...
  "WEB RESEARCH: when the task needs information from the web (not the current page or an attached file), plan ONLY `search_web` step(s) — one per distinct angle/sub-question. A deterministic research loop will then open the result links one at a time, read and extract each page, and gather the content for you, so you must NOT add `understand_page` steps to open or read result links yourself. Plan the searches; the loop reads the pages.",
